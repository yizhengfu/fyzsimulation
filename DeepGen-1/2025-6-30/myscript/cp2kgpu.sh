...
for inp_file in *.inp; do
    ...
    # 运行计算，保持原有的MPI参数
    # mpirun -np $SLURM_NTASKS cp2k.psmp -i "$inp_file" > "${base_name}.out" 2>&1
    mpirun -np $SLURM_NTASKS --bind-to none -x MKL_THREADING_LAYER=INTEL cp2k.psmp -i "$inp_file" > "${base_name}.out" 2>&1
