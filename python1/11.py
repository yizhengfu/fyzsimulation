...
L = 11.858
nLi, nCl = 32, 32
Li, Cl = get_atom_positions('./LiCl_DP_Tutorial_Example/chapter2/abacus_md_bak/OUT.ABACUS/MD_dump', nLi, nCl)
...
for (label, (x, y)), color in zip(atom_pairs.items(), colors):
    ...
    np.savetxt(f'./LiCl_DP_Tutorial_Example/chapter2/abacus_md_bak/OUT.ABACUS/gr_{label}', np.column_stack((rmesh, gr)))

plt.legend()
plt.title('abacus_md_rdf')
plt.savefig('./LiCl_DP_Tutorial_Example/chapter2/abacus_md_bak/abacus_md_rdf.png', dpi=300)
plt.show()