...
#define Natomtypes 18 //C,S,O,H,P,W,Mg,B,Li,F,Al,Si,As,Se,Fe,Cu,Au
...
#define Nitrogen 3
#define Hydrogen 4
#define Phosphorus 5   //用P元素代表弹丸中的C
#define Wolfram 6
#define Magnesium 7
#define Boron 8
#define Lithium 9
 ...
 while(fscanf(fp,"%s",buffer)!=EOF) {
   ...
   if(strcmp(buffer,"particles")==0) {
	...
	if (itemp == Carbon){
	  ...
	  sprintf(buffer, "N");
	  }else if(itemp == Sulfur) {
	  sprintf(buffer, "S");     
		}else if(itemp == Phosphorus) {
	  sprintf(buffer, "P");
    }else if(itemp == Wolfram) {
	  sprintf(buffer, "W");
    }else if(itemp == Magnesium) {
	  sprintf(buffer, "Mg");
    }else if(itemp == Boron) {
	  sprintf(buffer, "B");
    }else if(itemp == Lithium) {
	  sprintf(buffer, "Li");
    }else if(itemp == Fluorine) {
	  sprintf(buffer, "F");
    }else if(itemp == Aluminium) {
	  sprintf(buffer, "Al");
    }else if(itemp == Silicom) {
	  sprintf(buffer, "Si");
    }else if(itemp == Arsenic) {
	  sprintf(buffer, "As");
    }else if(itemp == Selenium) {
	  sprintf(buffer, "Se");
    }else if(itemp == Iron) {
	  sprintf(buffer, "Fe");
    }else if(itemp == Copper) {
	  sprintf(buffer, "Cu");
    }else if(itemp == Gold) {
	  sprintf(buffer, "Au");
	} 
...
/* This subroutine identifies the atom type from the config file and sets the masses.  Why set the masses here, I don't know.*/
  ...
  Mass[Sulfur]=32.0640;
  Mass[Phosphorus]=30.9738;
  Mass[Wolfram]=183.8000;
  Mass[Magnesium]=24.3050;
  Mass[Boron]=10.8110;
  Mass[Lithium]=6.9410;
  Mass[Fluorine]=18.9980;
  Mass[Aluminium]=26.9820;
  Mass[Silicom]=28.0850;
  Mass[Arsenic]=74.9216;
  Mass[Selenium]=78.9000;
  Mass[Iron]=55.8400;
  Mass[Copper]=63.5400;
  Mass[Gold]=196.9670;
  ...
  else if(strcmp(symbol,"S")==0) value=Sulfur;
  else if(strcmp(symbol,"P")==0) value=Phosphorus;
  else if(strcmp(symbol,"W")==0) value=Wolfram;
  else if(strcmp(symbol,"Mg")==0) value=Magnesium;
  else if(strcmp(symbol,"B")==0) value=Boron;
  else if(strcmp(symbol,"Li")==0) value=Lithium;
  else if(strcmp(symbol,"F")==0) value=Fluorine;
  else if(strcmp(symbol,"Al")==0) value=Aluminium;
  else if(strcmp(symbol,"Si")==0) value=Silicom;
  else if(strcmp(symbol,"As")==0) value=Arsenic;
  else if(strcmp(symbol,"Se")==0) value=Selenium;
  else if(strcmp(symbol,"Fe")==0) value=Iron;
  else if(strcmp(symbol,"Cu")==0) value=Copper;
  else if(strcmp(symbol,"Au")==0) value=Gold; 
  else {
